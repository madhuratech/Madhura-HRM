...
import { Bike, DollarSign, Calendar, User, Save, Loader2 } from 'lucide-react';
...
export function SalesEntry() {
  ...
  const fetchSales = async () => {
    ...
    try {
      ...
      setRecentSales(res.data.data.slice(0, 5)); // show latest 5
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="pt-4 border-t border-slate-100 flex justify-end">
              <button
                ...
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold py-2.5 px-6 rounded-lg shadow-md hover:shadow-lg transition-all flex items-center gap-2">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-slate-100 text-sm">
                {loading ? (
                  <tr><td colSpan="4" className="text-center p-4"><Loader2 className="animate-spin mx-auto text-blue-500" /></td></tr>
                ) : recentSales.length === 0 ? (
                  <tr><td colSpan="4" className="text-center p-4 text-slate-500">No recent sales</td></tr>
                ) : (
                  recentSales.map((sale) => (
                    <tr key={sale._id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-3 text-slate-800 font-medium">{sale.customerName}</td>
                      <td className="p-3 text-slate-600">{sale.model}</td>
                      <td className="p-3 text-slate-500">{new Date(sale.date).toLocaleDateString()}</td>
                      <td className="p-3 text-slate-800 font-bold text-right">${sale.amount.toLocaleString()}</td>
                    </tr>
                  ))
                )}
              </tbody>
